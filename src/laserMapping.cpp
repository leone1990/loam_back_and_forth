...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ...
    if (newLaserCloudLast && newLaserOdometry && fabs(timeLaserCloudLast - timeLaserOdometry) < 0.005) {
      ...
      VoxelFilter<pcl::PointXYZHSV> filter1(laserCloudCorner);
      filter1.SetLeafSize(0.05);
      filter1.ApplyFilter(*laserCloudCorner2);
     
      /*laserCloudCorner2->clear();
      //pcl::PointCloud<pcl::PointXYZHSV>::Ptr cloud_OPM_filtered(new pcl::PointCloud<pcl::PointXYZHSV>());
      pcl::VoxelGrid<pcl::PointXYZHSV> downSizeFilter;
      downSizeFilter.setInputCloud(laserCloudCorner);
      downSizeFilter.setDownsampleAllData(false);
      ...
      */
      ...
      VoxelFilter<pcl::PointXYZHSV> filter2(laserCloudSurf);
      filter2.SetLeafSize(0.1);
      filter2.ApplyFilter(*laserCloudSurf2);
